...
func main() {
	...
	// sourceDirectory := flag.String("sourcedir", "", "path to directory with source JSON documents")
...
func parseSourceFile(defaults esbulk.Options, path string) (esbulk.Options, error) {
	if _, err := os.Stat(path); os.IsNotExist(err) {
		return esbulk.Options{}, fmt.Errorf("failed to load %q as it is not IDL file", path)
	}
	if filepath.Ext(path) != ".ldj" {
		fmt.Print("wrong extension")
		return esbulk.Options{}, fmt.Errorf("failed to load %q as it is not IDL file", path)
	}
